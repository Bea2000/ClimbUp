...
export default function ParticipantProblemRow({ participant, problems, competitionId, setParticipants }: ParticipantProblemRowProps) {
  const [selectedProblem, setSelectedProblem] = useState<Problem>(problems[0]);
  const [selectedCompleted, setSelectedCompleted] = useState<boolean>(false);
  const [isConfirming, setIsConfirming] = useState(false);
  const [participantProblem, setParticipantProblem] = useState<Partial<ParticipantProblem> | null>(null);

  useEffect(() => {
    if (participant.competition.problems.find((problem) => problem.problemId === selectedProblem?.id)) {
      const problem = participant.competition.problems.find((problem) => problem.problemId === selectedProblem?.id);
  ...
  }, [selectedProblem, participant.competition.problems, participant.competition.id, participant.id]);

  function handleConfirm(participantId: number) {
    setIsConfirming(true);
    addProblemInformationToParticipant(participantId, selectedProblem.id, competitionId, selectedCompleted);
    ...
    <tr key={participant.id}>
      <td>
        <div className="text-sm">
          {participant.competition.userInformation && (
            Object.entries(participant.competition.userInformation as Record<string, string>)
      ...
      <td>
        <div className="badge badge-primary">
          {participantProblem?.attempts} / {selectedProblem.attempts}
        </div>
      </td>
      <td>
        <select
          className="select select-bordered w-full max-w-xs"
          onChange={(e) => setSelectedProblem(problems.find((problem) => problem.id === parseInt(e.target.value)) || problems[0])}
